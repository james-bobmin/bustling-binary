---
title: what is the x (x-rayvision (o.o)) in mdx
description: all this and more, coming up
publishedDate: 2024-10-26
tags:
  - markdown
  - TIL
  - SNL
---

## what is the x (x-rayvision (o.o)) in mdx

I was able to get some clothes and food with my birthday money and gifts, very grateful for all the support in the world.
![Porkbelly SupA beautiful sunset](/src/assets/Foodie.MP.JPG)

1. This is the rear of the ship, you aren't allowed in here.

2. But, I've been a good boy.

3. So far, so good.

4. again? See you in, spatbags.

```javascript /directory/description/this
const = 34343;
```

> pretty sure I registered a Domain this morning, stay tuned for spiritual perfection

:joy:

Not going to lie ~~b0b knows~~ lord knows what I am ==doing==

- [x] trampled the undead
- [x] uni-tard
- [ ] pygmy forest dongs :tent:
      ![Nordlys logo](@/assets/thatwasmythought.jpg)

### Images in MDX it's probably extended, it's always extended with these computer guys

If this is a midlife crisis I think I will be fine, will you?
